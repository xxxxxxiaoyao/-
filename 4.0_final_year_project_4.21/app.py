# 导入模块区域。
from flask import Flask, render_template, request, redirect, url_for, session
from model import db, User, Question, Option, Response, Feedback, Admin, Notification, Tracking, SRecord
from security import encrypt_password,decrypt_password
import sqlite3
from datetime import datetime
import random  # 导入 random 模块
import string  # 如果您使用了 string.digits，也需要导入 string

# 通用日志函数
# 用于记录系统操作日志
def write_log(action, user=None):
    log = SRecord(action=action, user=user)
    db.session.add(log)
    db.session.commit()




# 创建Flask应用区域。
app = Flask(__name__) #创建Flask应用实例
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///database.db' #配置数据库URL，使用的是SQLite数据库，文件名为database.db
app.secret_key = 'your_secret_key' #设置应用的密钥，用于会话加密和CSRF保护
db.init_app(app) #初始化数据库，将数据库实例与Flask应用关联起来



# 创建数据库表区域。
with app.app_context(): #创建应用上下文，确保数据库操作在正确的上下文中执行
    db.create_all() #创建所有定义的数据库表格


# 路由区域
# 注册路由区域。处理用户注册请求，支持GET和POST方法。GET方法渲染注册页面，POST方法处理注册逻辑。
@app.route('/register', methods=['GET', 'POST'])
def register():
    if request.method == 'POST':
        username = request.form['username']
        password = request.form['password']
        age = request.form['age']
        education_level = request.form['education_level']
        email = request.form['email']

        # print(f"Received username: {username}, password: {password}")
        encrypted_password = encrypt_password(password)
        # print(f"Encrypted password: {encrypted_password}")
        
        new_user = User(username=username, password=encrypted_password, age=age, education_level=education_level, email=email)
        db.session.add(new_user)
        db.session.commit()

        return redirect(url_for('login'))

    return render_template('register.html')

# 创建根路径路由
@app.route('/')
def home():
    return render_template('home.html')

# 创建登录路由
@app.route('/login', methods=['GET', 'POST']) #定义/login路由，支持GET和POST请求
def login():
    # 从表单中获取用户输入的用户名和密码
    if request.method == 'POST':
        input_username = request.form['username']
        input_password = request.form['password']

        # 查找用户
        user = User.query.filter_by(username=input_username).first()

        if user:
            try:
                # 解密数据库中加密的密码
                decrypted_password = decrypt_password(user.password)

                if decrypted_password == input_password:
                    session['user_id'] = user.id  # 记录用户登录状态
                    write_log("用户登录成功", user=input_username)
                    return redirect(url_for('home'))
                   
                else:
                    return "密码错误，请重试。"
                 
            except Exception as e: #这里捕获异常，处理解密过程中可能出现的错误
                return f"解密失败，可能是密钥文件缺失或错误：{str(e)}"
        else:
            return "用户名不存在。"
    




    return render_template('login.html')

# 评估路由区域
@app.route('/submit_survey', methods=['GET', 'POST'])
def submit_survey():
    if 'user_id' not in session:
        return redirect(url_for('login'))
    
    #建议文本字典
    reason_low = {
    "学习能力": "你的学习能力较弱。英国研究生一年内需要完成7门课程，学习强度高，建议提升自律与理解能力。",
    "语言能力": "语言能力是留学基础，0分说明你可能尚未掌握听说读写，建议通过系统学习补强。",
    "适应能力": "适应能力偏低，面对文化、饮食、教学方式的变化时可能感到不适，建议提前了解英国环境。",
    "社交能力": "低社交能力可能导致孤独、沟通不畅，建议培养英语口语及交际信心。",
    "用户背景": "出国留学对家庭背景和经济支持有较高要求，请结合自身资源谨慎评估。",
    "主观能动性": "缺乏主观能动性会导致跟不上课程进度，建议培养主动获取信息的习惯。"
    }
    reason_mid = {
    "学习能力": "你的学习能力处于一般水平，适应英国节奏仍需努力，可提前预习课程。",
    "语言能力": "你的语言基础尚可，但在学术写作或口语表达方面仍需提升。",
    "适应能力": "你具备一定适应能力，但建议出发前做好心理准备与文化适应训练。",
    "社交能力": "你具备基本社交能力，可通过参与社团等进一步建立人脉。",
    "用户背景": "你具备部分留学资源，建议提前准备好资金规划与住宿安排。",
    "主观能动性": "具备一定主观能动性，未来可多参加自学项目增强学习驱动力。"
    }
    reason_high = {
    "学习能力": "你的学习能力很强，能够应对英国高强度课程，非常适合出国深造。",
    "语言能力": "你的语言能力非常优秀，能轻松适应英语教学环境。",
    "适应能力": "你拥有优秀的适应能力，相信你能很快融入英国的学习与生活。",
    "社交能力": "你的社交能力出色，有助于建立人际关系和团队合作。",
    "用户背景": "你具备良好的家庭和经济支持，是非常有潜力的留学生候选人。",
    "主观能动性": "你极具主观能动性，这将帮助你在英国高自主性的学习环境中快速成长。"
}

    # 获取当前时间和开始时间
    from datetime import datetime
    end_time = datetime.utcnow()  # 获取当前时间
    global start_time  # 使用全局变量存储的开始时间
    time_taken = (end_time - start_time).total_seconds()  # 计算时间差（以秒为单位）

    
    #用户填写的内容传至数据库
    conn = sqlite3.connect('instance/database.db')#连接数据库
    cursor = conn.cursor()

    # 从表单数据中获取用户 ID
    userid = session.get('user_id')
    if not userid:
        return "用户 ID 缺失，请重新登录并提交评估。", 400
    # 打印表单数据
    # print("表单数据:", request.form)

    # 遍历表单数据
    for questionID, optionID in request.form.items():

        questionID = int(questionID)  # 问题 ID
        optionID = int(optionID)  # 选项 ID
        r_time = time_taken  # 使用计算的时间差
        cursor.execute("""
            INSERT INTO response (id, questionID, optionID, r_time)
            VALUES (?, ?, ?, ?)
        """, (userid, questionID, optionID,  r_time))


    # print("接受数据:",userid, questionID, optionID, r_time)

    conn.commit()# 提交事务,将插入的数据保存到数据库中
    conn.close()# 关闭数据库连接

    # 假设每个维度的权重如下（权重总和为1）
    dimension_weights = {
        '用户背景': 0.2,
        '学习能力': 0.15,
        '语言能力': 0.25,
        '适应能力': 0.1,
        '社交能力': 0.2,
        '主观能动性': 0.1
}


    # 处理用户提交的问卷，给分数
    if request.method == 'POST':
        form_data = request.form # 获取用户提交的表单数据，包括问题ID和选项ID
        scores_by_dimension = {} # 用于存储每个维度的分数
        question_scores = [] #创建一个列表用于保存“每道题的得分”

        #遍历用户提交的内容
        for question_id, option_id in form_data.items():
            question = Question.query.get(int(question_id))
            option = Option.query.get(int(option_id))
            
            #把这道题的题目和得分记录进列表
            question_scores.append({ #append()方法用于在列表末尾添加新的元素
                'question_id': question_id,
                'score': option.score
                })

            # 分类加分
            dimension = question.category
            scores_by_dimension.setdefault(dimension, 0) 
            # setdefault(dimension, 0)是一个字典方法，用于获取指定键的值，如果键不存在，则返回默认值0
            scores_by_dimension[dimension] += option.score

        # 这里我们假设每个维度满分是 10 分，标准化为百分制
        standardized = {
            dim: min(100, v / 10 * 100) for dim, v in scores_by_dimension.items()
        }

        #根据得分区间输出建议
        reason_output = []

        for dim, score in standardized.items():
            if score < 40:
                reason_output.append({"dimension": dim, "reason": reason_low[dim]})
                # 将一个字典添加到reason_output列表中，字典包含维度和对应的建议
                # dimension是维度名称，score是对应的分数
            elif score < 80:
                reason_output.append({"dimension": dim, "reason": reason_mid[dim]})
            else:
                reason_output.append({"dimension": dim, "reason": reason_high[dim]})


        if not standardized:
            return "未提交任何有效数据，请重新填写问卷。", 400

        # 计算加权总分
        total_score = sum(
            standardized[dim] * dimension_weights.get(dim, 0) for dim in standardized
            )


        return render_template(
            "result.html", 
            scores=standardized, 
            total_score=round(total_score, 2),
            question_scores=question_scores,
            reason_output=reason_output 
            )

    # GET 请求：展示问卷
    questions = Question.query.all()
    return render_template("assessment.html", questions=questions)

# 评估题目生成路由
@app.route('/survey')
def survey():
     # 检查用户是否已登录
    if 'user_id' not in session:
        return redirect(url_for('login'))  # 如果未登录，重定向到登录页面
  
     # 记等一下录用户开始评估的时间（直接获取系统时间）
    global start_time  # 使用全局变量存储开始时间
    start_time = datetime.utcnow()  # 使用 UTC 时间，避免时区问题
    
    # 连接数据库
    conn = sqlite3.connect('instance/database.db') 
    cursor = conn.cursor()

    # 获取所有题目
    cursor.execute("SELECT QuestionID, Question FROM assessment")
    questions = cursor.fetchall()
    # print(f"问题: {questions}")

    # 获取所有选项（通用选项）
    cursor.execute("SELECT optionID, content_of_option, score FROM option")
    common_options = cursor.fetchall()
    # print(f"选项: {common_options}")

    conn.close()

    # 整理成字典结构
    question_list = []
    for q in questions:
        q_id, q_content = q
        question_list.append({
            'id': q_id,
            'content': q_content,
            'options': common_options  # 所有问题使用相同的选项
        })
        # print(f"问题ID: {q_id}, 内容: {q_content}, 选项: {common_options}")
    


    return render_template('assessment.html', questions=question_list)

#历史记录路由
@app.route('/history')
def history():
    user_id = session.get('user_id')
    if not user_id:
        return redirect(url_for('login'))

    records = Response.query.filter_by(id=user_id).order_by(Response.responseid.desc()).all()
    
    
    return render_template('history.html', records=records)

# 个人资料路由
@app.route('/profile')
def profile():
    user_id = session.get('user_id')
    if not user_id:
        return redirect(url_for('login'))

    user = User.query.get(user_id)
    return render_template('profile.html', user=user)

# 重置密码路由
@app.route('/change_password', methods=['GET', 'POST'])
def change_password():
    user_id = session.get('user_id')
    if not user_id:
        return redirect(url_for('login'))

    user = User.query.get(user_id)

    if request.method == 'POST':
        old_password = request.form['old_password']
        new_password = request.form['new_password']
        confirm_password = request.form['confirm_password']

        # 解密并验证旧密码
        try:
            decrypted = decrypt_password(user.password)
        except Exception:
            return "解密出错，请联系管理员。"

        if decrypted != old_password:
            return "旧密码错误，请重试。"

        if new_password != confirm_password:
            return "两次输入的新密码不一致，请重新输入。"

        # 加密新密码并保存
        user.password = encrypt_password(new_password)
        db.session.commit()

        return redirect(url_for('profile'))

    return render_template('change_password.html')

# 忘记密码路由1
@app.route('/forgot_password', methods=['GET', 'POST'])
def forgot_password():
    if request.method == 'POST':
        email = request.form['email']
        user = User.query.filter_by(email=email).first()

        if not user:
            return "邮箱不存在，请检查输入。"

        # 生成验证码
        code = ''.join(random.choices(string.digits, k=6)) 
        # join()函数用于将列表中的字符连接成一个字符串，生成的验证码是一个6位数的字符串。
        # random.choices（）函数用于从指定的字符集中随机选择指定数量的字符，这里是从数字字符中随机选择6个字符，生成一个6位数的验证码。
        # string.digits 表示数字字符集（0-9），k=6 表示选择6个字符。
        session['reset_code'] = code
        session['reset_user_id'] = user.id

        # 模拟“发送邮件” —— 暂时打印到终端（部署时可替换为真实发邮件）
        print(f"验证码已发送到邮箱：{email}，验证码是：{code}")

        return redirect(url_for('reset_password'))

    return render_template('forgot_password.html')

# 忘记密码路由2（验证码 + 新密码）
@app.route('/reset_password', methods=['GET', 'POST'])
def reset_password():
    if request.method == 'POST':
        code = request.form['code']
        new_password = request.form['new_password']
        confirm = request.form['confirm_password']

        if code != session.get('reset_code'):
            return "验证码错误。"

        if new_password != confirm:
            return "新密码不一致。"

        user_id = session.get('reset_user_id')
        user = User.query.get(user_id)

        user.password = encrypt_password(new_password)
        db.session.commit()

        # 清除 session
        session.pop('reset_code', None)
        session.pop('reset_user_id', None)

        return redirect(url_for('login'))
    

    return render_template('reset_password.html')

#用户反馈路由
@app.route('/feedback', methods=['GET', 'POST'])
def feedback():
    user_id = session.get('user_id')
    if not user_id:
        return redirect(url_for('login'))

    if request.method == 'POST': 
        content = request.form['content']
        fb = Feedback(id=user_id, content=content) #创建一个新的Feedback对象fd
        db.session.add(fb)
        db.session.commit()
        return redirect(url_for('feedback'))

    # 展示当前用户历史反馈
    all_feedback = Feedback.query.filter_by(id=user_id).order_by(Feedback.submitted_at.desc()).all()
    return render_template('feedback.html', feedbacks=all_feedback)

# 管理员后台查看所有反馈
@app.route('/admin/feedback', methods=['GET', 'POST'])
def admin_feedback():
    if request.method == 'POST':
        feedback_id = int(request.form['feedback_id'])
        reply = request.form['reply']
        fb = Feedback.query.get(feedback_id)
        db.session.commit()
        return redirect(url_for('admin_feedback'))

    feedbacks = Feedback.query.order_by(Feedback.submitted_at.desc()).all()
    return render_template('admin_feedback.html', feedbacks=feedbacks)


# 管理员登录
@app.route('/admin_log', methods=['GET', 'POST'])
def admin_log():
    if request.method == 'POST':
        username = request.form['username']
        password = request.form['password']
        admin = Admin.query.filter_by(username=username, password=password).first()

        if not admin:
            return "您不是超级管理员。"
        return render_template('admin.html')
        


    return render_template('admin_log.html')

# 用户查看所有通知
@app.route('/notifications')
def notifications():
    all_notes = Notification.query.order_by(Notification.created_at.desc()).all()
    return render_template('notifications.html', notifications=all_notes)

# 管理员发布通知
@app.route('/admin/notifications', methods=['GET', 'POST'])
def admin_notifications():
    if request.method == 'POST':
        title = request.form['title']
        content = request.form['content']
        note = Notification(title=title, content=content)
        db.session.add(note)
        db.session.commit()
        return redirect(url_for('admin_notifications'))

    all_notes = Notification.query.order_by(Notification.created_at.desc()).all()
    return render_template('admin_notifications.html', notifications=all_notes)

# 管理员后台路由
@app.route('/admin')
def admin():
    return render_template('admin.html')


# 模拟发送邮件
@app.route('/tracking_send_feedback')
def send_tracking_emails():
    users = User.query.all()

    for user in users:
        feedback_url = url_for('tracking_feedback', email=user.email, _external=True)

   
        print("----- 模拟邮件发送 -----")
        print(f"发送给：{user.email}")
        print("邮件内容：")
        print("您好，感谢您使用我们的留学适应评估系统。")
        print("请点击以下链接填写结果验证调查问卷：")
        print(feedback_url)
        print("------------------------")

    return "所有邮件已模拟发送完毕。请查看终端输出。"


# 追踪路由
@app.route('/tracking_feedback', methods=['GET', 'POST'])
def tracking_feedback():
    email = request.args.get('email')

    if request.method == 'POST':
        user_email=email
        accuracy = request.form['accuracy']
        suggestion = request.form['suggestion']

        fb = Tracking(
            email=user_email,
            accuracy_opinion=accuracy,
            suggestion=suggestion
        )
        db.session.add(fb)
        db.session.commit()

        return "感谢您的反馈！我们已收到您的意见。"

    return render_template('tracking_feedback.html')

# 管理员查看系统日志
@app.route('/admin/logs')
def admin_logs():
    logs = SRecord.query.order_by(SRecord.timestamp.desc()).limit(100).all()
    return render_template("admin_logs.html", logs=logs)


# 启动区域
if __name__ == '__main__': #确保只有在直接运行此文件时才启动应用，而不是在被其他文件导入时。
    app.run(debug=True) # 启动Flask应用，debug=True表示启用调试模式，便于开发和调试

